...
def boltzmann_dist(Es, A, B, KT=1.):
    ''' '''
    ...
    S = .5*(np.sum(A.values())+ np.sum(B.values()))

    return num/(S*den)

stat_dist = total_var_dist

def compare_sols(A, B, label, r=3, plot=True):
    '''Compare the energy distributions of two DWAVE_Sol objects'''

    # energy occurances
    e_occs = {'A': defaultdict(int), 'B': defaultdict(int)}

    for i, e in enumerate(A.energies):
        e_occs['A'][round(e,r)] += A.occ[i]

    for i, e in enumerate(B.energies):
        e_occs['B'][round(e,r)] += B.occ[i]

    Es = sorted(set(e_occs['A']).union(e_occs['B']))

    dist = boltzmann_dist(Es, e_occs['A'], e_occs['B'])

    if plot:
        # plot both distributions
        X = np.arange(len(Es))
        w = .3
        Y = {'A': [e_occs['A'][e] for e in Es],
             'B': [e_occs['B'][e] for e in Es]}
        plt.figure(label)
        plt.bar(X-.75*w, Y['A'], width=.5*w, color=(1, 0, 0), log=True)
        plt.bar(X+.25*w, Y['B'], width=.5*w, color=(0, 0, 1), log=True)
        # plt.title('Number of output occurances', fontsize=FS)
        plt.legend([label, 'm(AB)'], fontsize=FS)
        plt.xlabel('State index', fontsize=FS)
        plt.ylabel('Number of occurances', fontsize=FS)
        plt.xlim([-2*w, 20+2*w])
        if SAVE:
            plt.savefig(os.path.join(IMG_DIR, 'dist.eps'), bbox_inches='tight')
        plt.show()
